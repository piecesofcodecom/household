 ...
 * @extends {ActorSheet}
 */
export class HouseholdActorSheet extends HandlebarsApplicationMixin(ApplicationV2) { //foundry.appv1.sheets.ActorSheet {
  ...
  static PARTS = {
    body: {
      template: "systems/household/templates/actor/actor-sheet.hbs"
    }
  };
  static TABS = {
    primary: {
  ...
  activateListeners(html) {
    ...
    html.on('click', '.action-configuration', async (env) => {
      ...
      const dialog_action_config = await foundry.applications.handlebars.renderTemplate("systems/household/templates/dialog/dialog-actions.hbs", templateData);
    ...
    //Custom edit
    html.on('click', '.custom-edit', async (ev) => {
